...
# Negative binomial model: multivariable adjusted for core clinical set + biomarkers (25-75th percentile scaling)
...
# Negative binomial model: adjusted for all covariates
...
# Calculate RR in different unit increase (eg. age per 10 years)
...
for (i in 1:10){ 
  model <- glm.nb(
    Number_severe_asthma_attacks_during_followup ~ Eczema_0no_1yes_9999notknown +
      ACQ_baseline_score_mean + 
      FEV1_preBD_PCT_Baseline + 
      Treatment_step +
      FeNO_baseline_ppb + 
  ...
  null_model <- 
  update(model, . ~ . - Eczema_0no_1yes_9999notknown)
