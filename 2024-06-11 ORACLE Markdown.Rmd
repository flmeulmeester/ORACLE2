...
summary_table_FEV1r <- data_ORACLE %>%
  ...
  dplyr::summarize(
    min_FEV1r = min(FEV1_PCT_reversibility_postBD, na.rm = FALSE),
    max_FEV1r = max(FEV1_PCT_reversibility_postBD, na.rm = FALSE)) # STATOS 1 and 2

summary_table_adh <- data_ORACLE %>%
  dplyr::group_by(Enrolled_Trial_name) %>%
  dplyr::summarize(
    min_adh = min(Adherence_InTrial_quantity, na.rm = FALSE),
    max_adh = max(Adherence_InTrial_quantity, na.rm = FALSE)) # CAPTAIN
```
...
numeric <- c(1,5,7,12,13,16,25:27,29,34,38:53)
...
imp_data_ORACLE_final_20240416 <- mice(data_arm_age12.SIshort, m = 10, maxit = 30, method = method, seed = 123, predictorMatrix = predM, printFlag = FALSE)

imp_data_ORACLE_final_20240416$loggedEvents # check logged events
```

```{r, eval = FALSE}
# Check convergence of the imputed data sets
plot(imp_data_ORACLE_final_20240416)
```

```{r, eval = FALSE}
# Write and save as R object file
saveRDS(imp_data_ORACLE_final_20240416, file="imp_data_ORACLE_final_20240416.RData")
```
...
imp_data_ORACLE_final_COMP_NR = mice::complete(imp_data_ORACLE_final_20240416, action = "long")
imp_data_ORACLE_final_COMP = mice::complete(imp_data_ORACLE_final_20240416, action = "long")
```
...
numeric <- c(1,5,7,12,16,25:27,29,34,38:54)
...
for (i in 3:56) {  # Loop through columns 3 to 56
...
numeric <- c(3,7,9,14,18,27:29,31,36,40:56)
...
fit <- with(data=imp_data_ORACLE_final_20240416, exp=glm.nb(Number_severe_asthma_attacks_during_followup ~ FEV1_PCT_reversibility_postBD + FEV1_preBD_PCT_Baseline + ACQ_baseline_score_mean + Any_severe_attack_previous_12m_0no_1yes + Treatment_step + offset(Follow_up_duration_days) + as.factor(Enrolled_Trial_name)))
...
fit <- with(data=imp_data_ORACLE_final_20240416, exp=glm.nb(Number_severe_asthma_attacks_during_followup ~ FEV1_PCT_reversibility_postBD + FEV1_preBD_PCT_Baseline + ACQ_baseline_score_mean +  Any_severe_attack_previous_12m_0no_1yes + Treatment_step + FeNO_baseline_ppb + Blood_Eos_baseline_x10_9_cells_per_L_zeroreplaced + offset(Follow_up_duration_days) + as.factor(Enrolled_Trial_name)))
...
fit <- with(data=imp_data_ORACLE_final_20240416, exp=glm.nb(Number_severe_asthma_attacks_during_followup ~ FeNO_baseline_ppb + ACQ_baseline_score_mean + Any_severe_attack_previous_12m_0no_1yes + FEV1_preBD_PCT_Baseline + Treatment_step + Blood_Eos_baseline_x10_9_cells_per_L_zeroreplaced + offset(Follow_up_duration_days) + as.factor(Enrolled_Trial_name)))
...
spline <- ggplot(data = subset1, aes(x = mean_BEC, 
                                     y = predict(model_final, type="response"), 
...
# Save the plot to files
ggsave("SPLINE PLOT FEV1 REV final version 18-07-2024.png", plot_fev_rev, width = 8, height = 6, units = "in", dpi = 600)
```
...
ggsave("20240718 Density plot BEC by FenO no legend v3.png", dens_bec_feno, width = 8, height = 6, units = "in", dpi = 300) # Save the plot
```
...
ggsave("20240718 Density plot BEC no enrichment with legend v3.png", dens_bec_suppl, width = 8, height = 6, units = "in", dpi = 300)
```
...
dens_rev <- ggplot(subset1, aes(x = mean_REV)) +
  ...
  scale_x_continuous(breaks = c(0, 10, 20, 30, 40, 50, 60, 70, 75)) + # Use the same scale as spline curve
...
ggsave("20240717 Density plot FEV1 rev v3.png", dens_rev, width = 8, height = 6, units = "in", dpi = 300) # Save the plot
```
...
imp_data_ORACLE_final_COMP <- imp_data_ORACLE_final_COMP %>%
  mutate(Adh_p = Adherence_InTrial_quantity / 10.66014)

imp_data_ORACLE_final_COMP_NR <- imp_data_ORACLE_final_COMP_NR %>%
  mutate(Adh_p = Adherence_InTrial_quantity / 13.51923)
```
...
### The values if the 25th and 75th percentile differ between the datasets (systematically missings imputed or not imputed)
imp_data_ORACLE_final_COMP <- imp_data_ORACLE_final_COMP %>%
  mutate(FEV1_p = FEV1_preBD_PCT_Baseline / 20.0775)

imp_data_ORACLE_final_COMP_NR <- imp_data_ORACLE_final_COMP_NR %>%
  mutate(FEV1_p = FEV1_preBD_PCT_Baseline / 20.90325)
```
...
### The values if the 25th and 75th percentile differ between the datasets (systematically missings imputed or not imputed)
imp_data_ORACLE_final_COMP <- imp_data_ORACLE_final_COMP %>%
  mutate(FEV1rev_p = FEV1_PCT_reversibility_postBD / 17.78255)

imp_data_ORACLE_final_COMP_NR <- imp_data_ORACLE_final_COMP_NR %>%
  mutate(FEV1rev_p = FEV1_PCT_reversibility_postBD / 19.20979)
```
...
```{r, eval = FALSE}
## FeNO
quantile(imp_data_ORACLE_final_COMP$FeNO_baseline_ppb)
quantile(imp_data_ORACLE_final_COMP_NR$FeNO_baseline_ppb)
```

```{r, eval = FALSE}
## FeNO
imp_data_ORACLE_final_COMP <- imp_data_ORACLE_final_COMP %>%
  mutate(FeNO_plog = FeNO_baseline_ppb / 0.4771213)
...
```{r, eval = FALSE}
## BEC
quantile(imp_data_ORACLE_final_COMP$Blood_Eos_baseline_x10_9_cells_per_L_zeroreplaced)
quantile(imp_data_ORACLE_final_COMP_NR$Blood_Eos_baseline_x10_9_cells_per_L_zeroreplaced)
```

```{r, eval = FALSE}
## BEC
imp_data_ORACLE_final_COMP <- imp_data_ORACLE_final_COMP %>%
  mutate(BEC_plog = Blood_Eos_baseline_x10_9_cells_per_L_zeroreplaced / 0.4771213)
...
```{r, eval = FALSE}
## Total IgE
quantile(imp_data_ORACLE_final_COMP$Total_IgE, na.rm = TRUE)
quantile(imp_data_ORACLE_final_COMP_NR$Total_IgE)
```

```{r, eval = FALSE}
## Total IgE
imp_data_ORACLE_final_COMP <- imp_data_ORACLE_final_COMP %>%
  mutate(IgE_plog = Total_IgE / 0.8313098)

imp_data_ORACLE_final_COMP_NR <- imp_data_ORACLE_final_COMP_NR %>%
  mutate(IgE_plog = Total_IgE / 0.8520344)
```
